...
class DeckTestPathFinder:
    ...
    def run_and_compare(
        self,
        trnsys_version_path: TrnsysVersionPaths,
...
def compare_prt_files(
    result_file_path: _pl.Path,
    expected_result_file_path: _pl.Path,
    show_diffs: bool = False,
    file_type: _tp.Literal["timestep", "hourly", "monthly"] = "timestep",
    massflow_solver: bool = False,
):
    ...
    if missing_columns_in_df_1:
        ...
        errors.append(
            ValueError(
                f"Columns found in expected file, which are missing in the result file: " f"{missing_columns_in_df_1}"
            )
        )
    ...
    if missing_columns_in_df_2:
        ...
        errors.append(
            ValueError(
                f"Columns found in expected file, which are missing in the result file: " f"{missing_columns_in_df_2}"
            )
        )
    try:
        df_diff = df_1 - df_2
    ...
    try:
        if abs_tolerance and rel_tolerance:
            _pd.testing.assert_frame_equal(
                df_new, df_expected, atol=abs_tolerance, rtol=rel_tolerance, check_exact=False
            )
        elif abs_tolerance:
        ...
        try:
            if abs_tolerance and rel_tolerance:
                _pd.testing.assert_frame_equal(
                    df_new, df_expected, atol=abs_tolerance, rtol=rel_tolerance, check_dtype=False, check_exact=False
                )
            elif abs_tolerance:
                _pd.testing.assert_frame_equal(
                    df_new, df_expected, atol=abs_tolerance, check_dtype=False, check_exact=False
                )
            elif rel_tolerance:
                _pd.testing.assert_frame_equal(
                    df_new, df_expected, rtol=rel_tolerance, check_dtype=False, check_exact=False
                )
            else:
