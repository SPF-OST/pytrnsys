...
def createLines(output: _output.Output):
    ...
    line = rf" & Share of Investment & &&& {output.componentGroups.annuity:2.0f} /a " \
           rf"({output.componentGroups.annuity * 100 / output.annuity:2.0f}\%) \\"
    lines += line + "\n"

    e = output.electricity
    line = rf" & Share of Electricity & {e.costElecFix:.0f}+{e.costElecKWh:.2f}/kWh & " \
    ...
    line = rf" & Share of Maintenance & &&& {maintenanceCost:2.0f} /a" \
           rf" ({maintenanceCost * 100 / output.annuity:2.0f}\%)\\ "
    lines += line + "\n"

    for yc in output.yearlyCosts.factors:
        line = rf" & Share of {yc.name} & {yc.coeffs.offset:.0f}+{yc.coeffs.slope:.2f}/{yc.value.unit} " \
    ...
    line = rf" & Share of Residual Value &&& & {residualCost:2.0f} /a " \
    ...
    line = rf" Energy Generation Costs & Using annuity: &&& {output.heatGenerationCost * 100:2.2f} "\
           rf"& Rp./kWh \\"
    lines += line
