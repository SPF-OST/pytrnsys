...
class ProcessMonthlyDataBase:
    ...
    def calculatePipeLosses(self):
        self.qLossPipeSystem = self.qLossPipeSolarLoop + self.qPipeLossHPSink + self.qPipeLossDHW + self.qPipeLossSH
    ...
    def calculateEnergyBalance(self):
        ...
        for i in range(12):
            ...
            if self.useFixedControl:
                if self.neglectSolarController:
                    ...
                    self.pElControllerSolar[i] = (
                        self.fixedWSolarController * utils.getNumberOfDaysOfMonth(i + 1) * 24 / 1000
                    )  # kW

                self.pElControllerHp[i] = (
                    self.fixedWHpController * utils.getNumberOfDaysOfMonth(i + 1) * 24 / 1000
                )  # kW
            ...
            if self.qWcpHP[i] == 0.0:
                ...
                self.SPFhp[i] = self.qCondHP[i] / (
                    ...
                    + self.pumpHPsink[i]
                    + self.pumpHPsource[i]
                    + self.pElVentilatorHP[i]
                    + self.qAuxHeaterHp[i]
                    + self.pElControllerHp[i]
                )

            self.pElPu[i] = (
                self.pumpHPsink[i] + self.pumpHPsource[i] + self.pumpSolar[i] + self.pElController[i] + self.pumpDHW[i]
            )
            ...
            if self.existSolarLoop:
                if self.qUse[i] > 0:
                    self.fSolar[i] = min(100.0 * self.qSolarToTes[i] / self.qUse[i], 100.0)
                else:
                    self.fSolar[i] = 0.0
            ...
            if (self.pElTotalDis[i] + self.qWcpHP[i]) <= 0.0:
                ...
                print(
                    "SPF:%f qD:%f Pel:%f wcp:%f pen:%f aux:%f"
                    % (
                        self.SPFhpsWithDisPen[i],
                        self.qDemand[i],
                        self.pElTotalDis[i],
                        self.qWcpHP[i],
                        self.pElPenalty[i],
        ...
        self.yearSPFhp = self.yearQCondHp / (
            sum(self.qWcpHP)
            + sum(self.qAuxHeaterHp)
            + sum(self.pElVentilatorHP)
            + self.yearPumpSourceHp
            + self.yearPumpSinkHp
            + sum(self.pElControllerHp)
            + 1e-30
        )
        ...
        if self.existSolarLoop:
            ...
            self.yearCOPtSolar = 100.0 * (self.yearQSolarToTes + sum(self.qSolarToPcm)) / (sum(self.qSunToCol) + 1e-30)
            self.yearCOPeSolar = (self.yearQSolarToTes + sum(self.qSolarToPcm)) / (
                sum(self.pumpSolar) + sum(self.pElControllerSolar) + 1e-30
            )

            print(
                "COPt:%f solarToCs:%f solarToWs:%f qSunToCol:%f"
                % (self.yearCOPtSolar, sum(self.qSolarToPcm), self.yearQSolarToTes, sum(self.qSunToCol))
            )

        else:
            self.yearQSolarToTes = 0.0
            self.yearEta = 0.0
            self.yearFSolar = 0.0
            self.yearCOPtSolar = 0.0
            self.yearCOPeSolar = 0.0
    ...
    def plotMonthlyWeatherData(self, yearlyFactor=10, printData=False):
        try:
            self.nameWeatherDataPlotPdf = self.plot.plotMonthly2Bar(
                self.iTHorizontalkWPerM2,
                self.iTColkWPerM2,
        ...
        self.nameWeatherDataHoPlotPdf = self.plot.plotMonthly(
            self.iTHorizontalkWPerM2,
            "Solar radiation $[kWh/m^2]$",
            "weatherDataMonthly",
            yearlyFactor=yearlyFactor,
    ...
    def plotBuildingMonthlyEnergyBalance(self, yearlyFactor=10, useYear=True, printData=False, printImb=True):
        ...
        self.nameMonthlyBuiPlotPdf = self.plot.plotMonthlyBalance(
            inVar,
            outVar,
            legends,
            "$Q$ $[kWh]$",
    ...
    def plotMonthlyEnergyBalance(self):  # to be updated with new functions
        ...
        barHpLoss = plot.bar(
            ind - 0.5 * width, -qLossHp, width, color="c", bottom=-(qDHW + qSH + qLossTes + qLossPipeSystem)
        )

        barImbNeg = plot.bar(
            ind - 0.5 * width, imbNeg, width, color="k", bottom=-(qDHW + qSH + qLossTes + qLossPipeSystem + qLossHp)
        )
        ...
        plot.set_xticklabels(
            ("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Year/10"), fontsize=25
        )
    ...
    def plotSolarMonthlyEnergyBalance(self):
        ...
        plot.legend(
            (bar1[0], bar2[0], bar3[0], barImbNeg[0]),
    ...
    def plotSPFMonthly(self, printData=False):
        ...
        if (self.SPFhps == self.SPFhpsNoCool).all():
            self.nameSpfPdf = self.plot.plotMonthly(
                self.SPFhps, "$SPF_{SHP}$", "SPFMonthly", yearlyFactor, useYearlyFactorAsValue=True, printData=printData
            )
        else:
            ...
            self.nameSpfPdf = self.plot.plotMonthly2Bar(
                spf, spfNoCool, legend, "$SPF_{SHP}$", "SPFMonthly", yearlyFactor
            )

            self.nameSpfNoCoolPdf = self.plot.plotMonthly(
                self.SPFhps,
                "$SPF^{heatOnly}_{SHP}$",
                "SPFNoCoolMonthly",
                self.yearSPFhps,
            ...
            self.nameSpfCoolPdf = self.plot.plotMonthly(
                self.SPFhpsNoCool,
                "$SPF^{HC}_{SHP}$",
                "SPFCoolMonthly",
                self.yearSPFhpsNoCool,
                useYearlyFactorAsValue=True,
                printData=printData,
            )
    ...
    def plotCOPMonthly(self, printData=False):
        ...
        self.nameCopPdf = self.plot.plotMonthly(
            self.SPFhp, "$SPF_{HP}$", "COPMonthly", yearlyFactor, useYearlyFactorAsValue=True, printData=printData
        )
    ...
    def plotPelBalanceFile(self, printData=False):
        ...
        if sum(self.pElVentilatorHP) > 0.0:
        ...
        self.namePElMonthlyPdf = self.plot.plotMonthlyBalance(
            inVar,
            outVar,
            legends,
            "Energy Flows",
    ...
    def addLatexWeatherData(self):
        ...
        for i in range(12):
            line = "%s & %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
                ...
                self.iTHorizontalkWPerM2[i],
                self.iTColkWPerM2[i],
                self.iTHorizontalkWPerM2[i] * self.Acol / 1000.0,
                self.iTColkWPerM2[i] * self.Acol / 1000.0,
            )
            lines = lines + line
        ...
        lines = lines + line
        line = "& %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
            sum(self.iTHorizontalkWPerM2),
            sum(self.iTColkWPerM2),
            sum(self.iTHorizontalkWPerM2) * self.Acol / 1000.0,
            sum(self.iTColkWPerM2) * self.Acol / 1000.0,
        )
    ...
    def addLatexShDemand(self):
        ...
        for i in range(12):
            line = "%s & %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
                ...
                self.qSH[i],
                self.qDemandSh[i],
                self.qDemandSh[i] - self.qUseSh[i],
                self.pElShPenalty[i],
            )
            lines = lines + line
        ...
        line = "& %.1f & %.1f & %.1f & %.1f (%.1f%s)\\\\ \n" % (
            sum(self.qUseSh),
            sum(self.qDemandSh),
            sum(self.qDemandSh) - sum(self.qUseSh),
            sum(self.pElShPenalty),
            100.0 * sum(self.pElShPenalty) / sum(self.qUseSh),
            symbol,
        )
    ...
    def addLatexDhwDemand(self):
        ...
        for i in range(12):
            line = "%s & %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
                ...
                self.qUseDhw[i],
                self.qDemandDhw[i],
                self.qDemandDhw[i] - self.qUseDhw[i],
                self.pElDhwPenalty[i],
            )
            lines = lines + line
        ...
        line = "& %.1f & %.1f & %.1f & %.1f (%.1f%s)\\\\ \n" % (
            sum(self.qUseDhw),
            sum(self.qDemandDhw),
            sum(self.qDemandDhw) - sum(self.qUseDhw),
            sum(self.pElDhwPenalty),
            100.0 * sum(self.pElDhwPenalty) / sum(self.qUseDhw),
            symbol,
        )
    ...
    def addLatexHeatDemand(self):
        ...
        line = "& %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
            sum(self.qUse),
            sum(self.qDemand),
            sum(self.qDemand) - sum(self.qUse),
            sum(self.pElPenalty),
        )
    ...
    def addTablePumpWorkingHours(self):
        ...
        for i in range(12):
            line = "%s & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
                ...
                self.onOffPumpCol[i],
                self.onOffPumpHp[i],
                self.onOffPumpHpDhw[i],
                self.onOffPumpHpSh[i],
                self.onOffPumpSh[i],
                self.onOffPumpDhw[i],
            )
            lines = lines + line
        ...
        line = "& %.1f & %.1f & %.1f& %.1f & %.1f & %.1f\\\\ \n" % (
            sum(self.onOffPumpCol),
            sum(self.onOffPumpHp),
            sum(self.onOffPumpHpDhw),
            sum(self.onOffPumpHpSh),
            sum(self.onOffPumpSh),
            sum(self.onOffPumpDhw),
        )
    ...
    def addLatexGlobalBalance(self):
        ...
        for i in range(12):
            ...
            line = (
                ...
                % (
                    utils.getMonthKey(i + 1),
                    self.qSolarToSystem[i],
                    self.qAuxHeaterTotal[i] - self.qAuxHeaterHp[i],
                    self.qEvapHP[i],
                    self.qWcpHP[i],
                    self.qAuxHeaterHp[i],
                    self.qLossPipeSystem[i],
        ...
        line = "& %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f\\\\ \n" % (
            ...
            sum(self.qAuxHeaterTotal) - sum(self.qAuxHeaterHp),
            sum(self.qEvapHP),
            sum(self.qWcpHP),
            sum(self.qAuxHeaterHp),
            sum(self.qLossPipeSystem),
            sum(self.qLossTes),
            sum(self.qLossConTnk),
            sum(self.qLossHp),
            sum(self.qDHW),
            sum(self.qSH),
            Imb,
        ...
        lines = lines + line
        line = "\\textbf{$Imb$}& %.2f %s\\\\ \n" % (Imb * 100 / (sum(self.qUse) + 1e-30), symbol)
        lines = lines + line
    ...
    def addLatexHeatPumpData(self):
        ...
        lines = lines + line
        line = "\\textbf{$Imb$}& \\textbf{%.2f} %s\\\\ \n" % (sumImb * 100 / (sum(self.qCondHP) + 1e-20), self.perCent)
        lines = lines + line
    ...
    def addLatexSecundaryLoopData(self):
        ...
        for i in range(12):
            bal = (
                self.qCondHP[i]
                - (self.qTesDhwFromHp[i] + self.qTesShFromHp[i])
                + self.qOutFromTesToSH[i]
                - (self.qPipeLossHPSink[i] + self.qPipeLossSH[i])
                - self.qRadiator[i]
            )
        ...
        line = " & %.1f & %.1f  & %.1f  & %.1f & %.1f & %.1f & %.1f  \\\\ \n" % (
            sum(self.qCondHP),
            sum(self.qTesDhwFromHp) + sum(self.qTesShFromHp),
    ...
    def addLatexSolarDataSerial(self):
        ...
        for i in range(12):
            ...
            imb = (
                self.qSolarToSystem[i]
                - self.qLossPrimaryLoop[i]
                - self.qTesFromSolar[i]
                - self.qPcmFromSolar[i]
                - self.qEvapHpFromSolar[i]
            )

            Imb = imb + Imb

            line = (
                "%s & %.2f & %.2f & %.2f & %.2f & %.2f  & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\ \n"
                % (
                    utils.getMonthKey(i + 1),
                    self.qIrradHor[i],
                    self.qIrradTilt[i],
                    ...
                    self.qSolarCond[i],
                    self.fcondsolar[i],
                    max(self.qAmbient[i], 0),
                    max(self.fambsolar[i], 0.0),
                    self.qTesFromSolar[i],
                    self.qPcmFromSolar[i],
                    self.qEvapHpFromSolar[i],
                    self.etaCol[i],
        ...
        line = (
            ...
            % (
                ...
                sum(self.qSolarToSystem),
                sum(self.qSolarCond),
                100 / sum(self.qColHeatGain) * sum(self.qSolarCond),
                sum(self.qAmbient),
                100 / sum(self.qColHeatGain) * sum(self.qAmbient),
                sum(self.qTesFromSolar),
                sum(self.qPcmFromSolar),
                sum(self.qEvapHpFromSolar),
                self.yearEta,
                sum(self.qLossPrimaryLoop),
                self.yearFSolar,
                sum(self.qUse),
                sum(self.pumpSolar),
                sum(self.pElControllerSolar),
                Imb,
            )
        )
        lines = lines + line
    ...
    def addLatexSolarDataParallel(self):
        ...
        lines = lines + line
        line = (
            "&  $[kWh/m^2]$&  $[kWh/m^2]$  &  [kWh]  & [kWh] &   [kWh]  & [-] &   [kWh] &      [\\%] &  [kWh] \\\\ \n"
        )
        lines = lines + line
        ...
        for i in range(12):
            imb = (
                self.qSolarToSystem[i]
                - self.qLossPipeSolarLoop[i]
                - self.qSolarToTes[i]
                - self.qSolarToTnkBrine[i]
                - self.qSolarToIce[i]
            )
            Imb = imb + Imb
            ...
            line = "%s & %.2f & %.2f & %.2f  & %.2f & %.2f & %.2f & %.2f & %.2f& %.2f  \\\\ \n" % (
                utils.getMonthKey(i + 1),
                self.iTColkWPerM2[i],
                self.qSolarToSystemPerM2[i],
                self.qSolarToSystem[i],
                self.qSolarToTes[i],
                self.qSolarToTnkBrine[i] + self.qSolarToIce[i],
                self.etaCol[i],
                self.qLossPipeSolarLoop[i],
                self.fSolar[i],
                imb,
        ...
        line = " & %.2f & %.2f & %.2f  & %.2f  & %.2f & %.2f & %.2f & %.2f & %.2f\\\\ \n" % (
            sum(self.iTColkWPerM2),
            sum(self.qSolarToSystemPerM2),
            sum(self.qSolarToSystem),
            sum(self.qSolarToTes),
            sum(self.qSolarToTnkBrine) + sum(self.qSolarToIce),
            self.yearEta,
            sum(self.qLossPipeSolarLoop),
            self.yearFSolar,
            Imb,
        )
        lines = lines + line
    ...
    def addLatexStorageDataDhwOnly(self):
        ...
        for i in range(12):
            ...
            outTes = (
                ...
                + self.qOutFromDhwTesCirculation[i]
            )
            ...
            line = "%s  & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f& %.1f& %.1f & %.1f \\\\ \n" % (
                utils.getMonthKey(i + 1),
                self.qDhwTesFromSolar[i],
                self.qDhwTesFromHp[i],
                self.qDhwTesFromElRot[i],
                intoTes,
                self.qOutFromDhwTesToDHW[i],
                self.qLossDhwTes[i],
                self.qOutFromDhwTesCirculation[i],
        ...
        line = "Year & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f& %.1f& %.1f & %.1f \\\\ \n" % (
            ...
            sum(self.qDhwTesFromHp),
            sum(self.qAuxHeaterDhwTes),
            sumIn,
            sum(self.qOutFromDhwTesToDHW),
            sum(self.qLossDhwTes),
            sum(self.qOutFromDhwTesCirculation),
            sum(self.qDhwTesAcum),
            Imb,
            sum(self.qDHW),
        )
    ...
    def addLatexStorageData(self):
        ...
        for i in range(12):
            ...
            line = (
                "%s & %.1f & %.1f  & %.1f & %.1f  & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f \\\\ \n"
                % (
                    utils.getMonthKey(i + 1),
                    self.qTesFromSolar[i],
                    self.qTesFromHp[i],
    ...
    def addLatexBuildingBalanceTable(self):
        ...
        for i in range(12):
            pos = (
                ...
                + self.qBuiIntGainPeople[i]
                + self.qBuiIntGainEq[i]
                + self.qBuiIntGainLight[i]
            )
            ...
            line = (
                "%s & %.1f & %.1f & %.1f  & %.1f & %.1f  & %.1f  & %.1f& %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f  & %.1f\\\\ \n"
                % (
                    utils.getMonthKey(i + 1),
                    qDemandUsed[i],
                    self.qBuiSolarGains[i],
        ...
        line = (
            ...
            % (
                ...
                sum(self.qBuiIntGainPeople),
                sum(self.qBuiIntGainEq),
                sum(self.qBuiIntGainLight),
                sum(self.qBuiTransLosses),
                sum(self.qBuiInfLosses),
                sum(self.qBuiVentLosses),
                sum(self.qBuiGroundLosses),
                sum(self.qBuiAcum),
                Imb,
                sum(self.qBuiHeat),
                sum(self.qBuiCool),
