...
class _CollectTokensVisitorBase(_lvis.Visitor_Recursive, _abc.ABC):
    ...
    def _addComputedVar(self, tree: _lark.Tree, portProperty: str) -> None:
        portName = _getChildToken("PORT_NAME", tree)
        defaultVariableName = _getChildToken("DEFAULT_VARIABLE_NAME", tree, allowMissing=True)
...
class _WithoutPlaceholdersJSONCollectTokensVisitor(_CollectTokensVisitorBase):
    ...
    def equation(self, tree: _lark.Tree) -> None:
        ...
        defaultVariableName = _getChildToken("DEFAULT_VARIABLE_NAME", assignmentTarget, allowMissing=True)
        if defaultVariableName:
            self._addComputedVar(assignmentTarget, portProperty="@temp")
            return

        portName = _getChildToken("PORT_NAME", assignmentTarget)
...
def _getChildToken(tokenType: str, tree: _lark.Tree, allowMissing=False) -> _tp.Optional[str]:
    ...
    if nMatches == 0:
        ...
        raise ValueError(f"No token of type {tokenType} found.")
    if nMatches > 1:
...
def replaceTokensWithDefaults(inputDdckFilePath: _pl.Path) -> _res.Result[str]:
    ...
    defaultNamesForComputedVariables = [v.defaultVariableName for v in visitor.computedVariables]
...
def replaceTokens(
    ...
    computedNames = []
    for computedVariable in visitor.computedVariables:
        ...
        if not computedName:
            return _res.Error(
                f"Unknown property `{computedVariable.portProperty}` for port `{computedVariable.portName}` in "
